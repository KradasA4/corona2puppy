    ...
    window.setTimeout(function(){
        ...
        for (var i=0, max=all.length; i < max; i++) {
           var num = mySelection[Math.floor(Math.random()*mySelection.length)]
            if(all[i].innerHTML){all[i].innerHTML =  all[i].innerHTML.replace(/\bcoronavirus\b|\bcovid-19\b|\bcorona\b|\bcovid\b|\bvirus\b|\bquarantine\b/gi,  `<img src="${num}" alt="puppy gif" height="120" width="200">`)}}
